...
for catalog_asset in ingredients.json recipe_sources.json slovak_ingredient_forms.json recipes/manifest.json; do
...
# Mená ročnej live/test konfigurácie patria do release kontraktu, hodnoty však
# ostávajú iba v serverovom env súbore. Chýbajúce nastavenie pri OFF režime
# neblokuje bezpečné nasadenie kódu; readiness aj tak nepovolí nový checkout.
ANNUAL_CONFIG_MISSING=0
...
if [ "$ANNUAL_CONFIG_MISSING" -ne 0 ]; then
  log "ročná platobná konfigurácia nie je kompletná; checkout ostáva zatvorený"
fi
...
nasad_z() {   # $1 = adresár s vydaním
  ...
  TODAY_SK=$(TZ=Europe/Bratislava date +%F) || return 1
  "$PY" -u "$DIR/refresh_blocek.py" --publish-html \
    /var/lib/uvarsi/landing_data.json /var/www/uvarsi/index.html "$TODAY_SK" || return 1
