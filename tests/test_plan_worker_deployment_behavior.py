...
def deployment(tmp_path):
    ...
    write_executable(
        ...
        "service=${1:-}\n"
        "case \"$cmd\" in\n"
        "  is-enabled) [ \"$service\" = uvarsi ] || [ \"$(cat \"$UVARSI_FAKE_STATE/enabled\")\" = 1 ] ;;\n"
        "  is-active) [ \"$service\" = uvarsi ] || [ \"$(cat \"$UVARSI_FAKE_STATE/active\")\" = 1 ] ;;\n"
        "  enable) printf 1 > \"$UVARSI_FAKE_STATE/enabled\" ;;\n"
        "  disable) printf 0 > \"$UVARSI_FAKE_STATE/enabled\" ;;\n"
        "  start|restart) [ \"$service\" != uvarsi-plan-worker ] || printf 1 > \"$UVARSI_FAKE_STATE/active\" ;;\n"
        "  stop) [ \"$service\" != uvarsi-plan-worker ] || printf 0 > \"$UVARSI_FAKE_STATE/active\" ;;\n"
        "  daemon-reload) [ ! -f \"$UVARSI_FAKE_STATE/fail-reload\" ] ;;\n"
        "  *) exit 64 ;;\n"
        "esac\n",
    )
    curl = tmp_path / "curl"
...
def test_snapshot_restore_preserves_exact_prior_worker_state(
    ...
    snapshot = bash_path(deployment["snapshot"])

    saved = run_library(deployment, f'uvarsi_snapshot "{snapshot}"')
    ...
    restored = run_library(deployment, f'uvarsi_restore "{snapshot}"')
...
def test_snapshot_failure_aborts_and_restore_failure_propagates(deployment):
    ...
    failed_backup = run_library(deployment, f'uvarsi_snapshot "{snapshot}"')
    assert failed_backup.returncode != 0

    deployment["live"].joinpath("missing-app").rename(deployment["app"])
    assert run_library(deployment, f'uvarsi_snapshot "{snapshot}"').returncode == 0
    deployment["state"].joinpath("fail-reload").touch()
    failed_restore = run_library(deployment, f'uvarsi_restore "{snapshot}"')
...
def test_partial_live_mutation_rolls_back_before_returning_failure(deployment):
    ...
    result = run_library(
        deployment,
        f'uvarsi_install_core "{bash_path(release)}" "{snapshot}"',
    )
